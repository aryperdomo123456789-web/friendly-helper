...
-   **Otimização para Smart TVs & Universal (TV Ready)**: 
    -   **CSS Engine**: Estilos globais em `src/styles.css` com resets legacy e suporte a hardware acceleration (GPU) para evitar lag em CPUs de TVs.
    -   **Spatial Navigation**: O motor em `src/lib/tv-navigation.ts` gerencia o foco via D-Pad (Controle Remoto) automaticamente em todo o app.
    -   **Mobile Focus**: Resets de zoom para iOS e targets touch otimizados.
    -   **Auto-Scale**: Interface adaptativa que escala fontes para 18px-20px em telas 4K.
    -   **Navegação**: O hook `useTVNavigation` intercepta eventos de controle remoto e mapeia para navegação espacial por proximidade.
